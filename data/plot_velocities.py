...
# We would like to plot the x, y, z velocities of diff, goal, and current on diff plots, x on top, y in the middle, z on the bottom

fig, axs = plt.subplots(4, 1)

# show average velocity change
...
axs[1].set(xlabel='time', ylabel='goal/current',
               title='Goal/Current Velocity X, avg diff: {}'.format(avg_x))
axs[1].grid()
axs[1].legend()
...
axs[2].set(xlabel='time', ylabel='goal/current',
               title='Goal/Current Velocity Y, avg diff: {}'.format(avg_y))
axs[2].grid()
axs[2].legend()
...
axs[3].set(xlabel='time', ylabel='goal/current',
               title='Goal/Current Velocity Z, avg diff: {}'.format(avg_z))
axs[3].grid()
axs[3].legend()
