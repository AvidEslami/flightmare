...
def train_gamma(value, filepath):
...
def train_cliprange(value, filepath):
    args = parser().parse_args()
    cfg = YAML().load(open(os.environ["FLIGHTMARE_PATH"] +
                           "/flightlib/configs/vec_env.yaml", 'r'))
    if not args.train:
...
def train_ent_coef(value, filepath):
    args = parser().parse_args()
    cfg = YAML().load(open(os.environ["FLIGHTMARE_PATH"] +
                           "/flightlib/configs/vec_env.yaml", 'r'))
    if not args.train:
...
def main():
    for i in range(0.9, 1.0, 0.01):
        train_gamma(i, filepath="successes/Rewards_gamma.csv")

    for i in range(0.0, 0.3, 0.01):
        train_cliprange(i, filepath="successes/Rewards_gamma.csv")

    for i in range(0.01, 0.1, 0.01):
        train_ent_coef(i, filepath="successes/Rewards_gamma.csv")
